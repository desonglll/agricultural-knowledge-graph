...
class Neo4j:
    ...
    def connectDB(self):
        self.graph = Graph("http://localhost:7474", auth=("neo4j", "Lindesong7758?"))
    ...
    def findRelationByEntity(self, entity1):
        answer = self.graph.run(
            'MATCH (n1 {title:"' + str(entity1) + '"})- [rel] -> (n2) RETURN n1,rel,n2'
        ).data()
        # if(answer is None):
    ...
    def findRelationByEntity2(self, entity1):
        answer = self.graph.run(
            'MATCH (n1)- [rel] -> (n2 {title:"' + str(entity1) + '"}) RETURN n1,rel,n2'
        ).data()
